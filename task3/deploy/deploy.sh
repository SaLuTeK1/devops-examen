...
if [ "$ENVIRONMENT" = "dev" ]; then
  docker compose -p devops-exam-dev -f "${DEPLOY_DIR}/docker-compose.dev.yml" down || true
  ...
  docker compose -p devops-exam-release -f "${DEPLOY_DIR}/docker-compose.release.yml" down || true
